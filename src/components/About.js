export const About = () => {
  return (
    <aside className="contenedor-secundario row">
      <article className="col-12" id="about">
        <h2 className="text-center">Sobre CodeCat</h2>
        <p>
          CodeCat es un proyecto que nace de la idea de 4 jóvenes en el proyecto
          final de un bootcamp de programación.
        </p>
        <p>
          <b>Jose</b> Lorem, ipsum dolor sit amet consectetur adipisicing elit.
          Culpa pariatur, veniam nisi libero nihil modi debitis nemo vel
          consequuntur, quas ad. Cum quasi repellendus possimus ipsum, rerum
          dolorum id. Praesentium!
        </p>
        <p>
          <b>Albert</b> Lorem, ipsum dolor sit amet consectetur adipisicing
          elit. Culpa pariatur, veniam nisi libero nihil modi debitis nemo vel
          consequuntur, quas ad. Cum quasi repellendus possimus ipsum, rerum
          dolorum id. Praesentium!
        </p>
        <p>
          <b>Sandra</b> Lorem, ipsum dolor sit amet consectetur adipisicing
          elit. Culpa pariatur, veniam nisi libero nihil modi debitis nemo vel
          consequuntur, quas ad. Cum quasi repellendus possimus ipsum, rerum
          dolorum id. Praesentium!
        </p>
        <p>
          <b>Raúl</b> Lorem, ipsum dolor sit amet consectetur adipisicing elit.
          Culpa pariatur, veniam nisi libero nihil modi debitis nemo vel
          consequuntur, quas ad. Cum quasi repellendus possimus ipsum, rerum
          dolorum id. Praesentium!
        </p>
        <p></p>
      </article>
    </aside>
  );
};
